...
function registerServer(context: AgentContext) {
  ...
  for (const mod of MODULES.filter((m) => agentMetaOf(m).agentVisible && m.key !== "daily_task")) {
    ...
    const createFields = mod.fields.filter((f) => !f.internal && !f.updateOnly && meta.writeFields.includes(f.name)).map((f) => f.name);
    ...
    server.tool(`${mod.key}_create`, { description: `新增${mod.labelZh}`, inputSchema: { type: "object", properties: Object.fromEntries(createFields.map((f) => [f, { type: "string" }])) } , handler: async (p: any) => { try { return json({ ok: true, ...(await service.create(mod.key, p)) }); } catch (e) { return errorResult(e); } }});
