...
type TableDef = {
  table: string;
  label: string;
  listFilters?: { name: string; type: string; description: string }[];
  createFields: { name: string; type: string; required: boolean; description: string }[];
  updateFields?: { name: string; type: string; description: string }[];
};
...
const TABLES: Record<string, TableDef> = {
  pantry: {
    ...
    listFilters: [
      ...
      { name: "status", type: "string", description: "状态: 充足/即将过期/已过期" },
    ],
    createFields: [
      { name: "name", type: "string", required: true, description: "名称" },
      { name: "category", type: "string", required: true, description: "分类" },
      { name: "quantity", type: "string", required: false, description: "数量" },
      { name: "expiry_date", type: "string", required: false, description: "过期日期 YYYY-MM-DD" },
      { name: "purchase_date", type: "string", required: false, description: "购入日期" },
  ...
  schedule: {
    ...
    createFields: [
      ...
      { name: "importance", type: "string", required: false, description: "紧急/重要/普通/低" },
      { name: "status", type: "string", required: false, description: "未开始/进行中/已完成/已取消" },
  ...
  finance: {
    ...
    createFields: [
      ...
      { name: "exchange_rate", type: "number", required: false, description: "汇率" },
      { name: "notes", type: "string", required: false, description: "备注" },
    ],
  },
  todos: {
    ...
    createFields: [
      ...
      { name: "importance", type: "string", required: false, description: "重要性" },
      { name: "detail", type: "string", required: false, description: "详情" },
      { name: "is_completed", type: "boolean", required: false, description: "是否完成" },
    ],
  },
  thoughts: {
    table: "thoughts",
    ...
    createFields: [
      ...
      { name: "icon", type: "string", required: false, description: "装饰emoji" },
    ],
  },
};

// Build properties from field defs
function fieldsToProperties(fields: TableDef["createFields"]) {
...
// ─── Register CRUD tools for each module ───

for (const [module, def] of Object.entries(TABLES)) {
  // LIST
  const listProps: Record<string, any> = { limit: { type: "number", description: "最多返回条数(默认50)" } };
  for (const f of def.listFilters || []) {
    listProps[f.name] = { type: f.type, description: f.description };
  }
  mcpServer.tool({
    name: `${module}_list`,
    ...
    handler: async (params: any) => {
      ...
      if (module === "schedule" && params.start_date) q = q.gte("start_time", params.start_date);
      if (module === "schedule" && params.end_date) q = q.lte("start_time", params.end_date + "T23:59:59");
      if (module === "finance" && params.month) {
  ...
  mcpServer.tool({
    ...
    inputSchema: { type: "object", properties: { id: { type: "string", description: "记录UUID" } }, required: ["id"] },
    handler: async (params: any) => {
  ...
  // CREATE
  const requiredCreate = def.createFields.filter((f) => f.required).map((f) => f.name);
  mcpServer.tool({
    name: `${module}_create`,
    description: `新增${def.label}记录`,
    inputSchema: { type: "object", properties: fieldsToProperties(def.createFields), required: requiredCreate },
    handler: async (params: any) => {
  ...
  mcpServer.tool({
    ...
    inputSchema: {
      type: "object",
      properties: { id: { type: "string", description: "记录UUID" }, ...fieldsToProperties(def.createFields) },
      required: ["id"],
    },
    handler: async (params: any) => {
      ...
      for (const [k, v] of Object.entries(rest)) {
        if (v !== undefined) {
          if (k === "tags" && typeof v === "string") {
            updates[k] = (v as string).split(",").map((s) => s.trim());
          } else {
            updates[k] = v;
          }
        }
...
mcpServer.tool({
  ...
  inputSchema: {
    type: "object",
    properties: {
      year: { type: "number", description: "年" },
  ...
  handler: async (params: any) => {
    ...
    const { data, error } = await sb.from("finance_records").select("*")
    ...
    return {
      content: [{
        type: "text",
        text: JSON.stringify({ month: monthStr, total_cny: total, by_category: byCategory, record_count: (data || []).length }, null, 2),
      }],
    };
  },
});
...
mcpServer.tool({
  ...
  inputSchema: {
    type: "object",
    properties: { data: { type: "string", description: "JSON字符串（导出格式）" } },
    required: ["data"],
  },
  handler: async (params: any) => {
...
mcpServer.resource({
  ...
  handler: async () => {
    ...
    const [scheduleRes, calorieRes, financeRes, todoRes, pantryRes] = await Promise.all([
    ...
    const totalCalories = (calorieRes.data || []).reduce((s: number, r: any) => s + r.calories, 0);
    const totalSpent = (financeRes.data || []).reduce((s: number, r: any) => s + Number(r.amount_cny), 0);
    const urgentTodos = (todoRes.data || []).filter((t: any) => t.importance === "紧急").length;

    const threeDaysLater = new Date(Date.now() + 3 * 86400000).toISOString().split("T")[0];
    const expiringSoon = (pantryRes.data || []).filter((p: any) => p.expiry_date && p.expiry_date <= threeDaysLater).length;

    return JSON.stringify({
      today_events: (scheduleRes.data || []).length,
      next_event: (scheduleRes.data || [])[0]?.title || null,
      today_calories: totalCalories,
      month_spent_cny: totalSpent,
      pending_todos: (todoRes.data || []).length,
      urgent_todos: urgentTodos,
      expiring_pantry: expiringSoon,
    }, null, 2);
  },
});
...
mcpServer.resource({
  ...
  handler: async (uri: URL) => {
    ...
    const { data } = await sb.from("finance_records").select("*")
      .gte("date", monthStr + "-01").lte("date", monthStr + "-31");
    const total = (data || []).reduce((s: number, r: any) => s + Number(r.amount_cny), 0);
    const byCategory: Record<string, number> = {};
    for (const r of data || []) {
...
mcpServer.resource({
  ...
  handler: async (uri: URL) => {
    ...
    for (const r of data || []) {
      byMeal[r.meal_type] = (byMeal[r.meal_type] || 0) + r.calories;
    }
