...
export function buildDailyRuleCopy(input: {
  ...
  const { scores, profile, lang, overallPercent } = input;
  ...
  const pct = overallPercent ?? Math.round(40 + scores.overall * 10);

  if (lang === "en") {
    const meta = hasProfile ? `${z?.en || ""} · ${s?.en || ""}`.replace(/^ · | · $/g, "").trim() : "";
    const headline = `Overall ${pct}`;
  ...
  const meta = hasProfile
    ? [z?.zh, s?.zh ? `属${s.zh}` : ""].filter(Boolean).join(" · ")
    : "";
  const headline = `整体运势 ${pct}`;
  const body = hasProfile
