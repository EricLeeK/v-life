...
const api = vi.hoisted(() => ({ create: vi.fn(), update: vi.fn(), toast: vi.fn(), pending: false }));
...
vi.mock("@/hooks/useData", () => {
  ...
  return {
    ...
    useFinanceByMonth: () => ({ data: [{ id: "expense", name: "午餐", category: "餐饮", currency: "JPY", amount: 1000, amount_cny: 50, exchange_rate: 0.05, date: `${new Date().getFullYear()}-${String(new Date().getMonth() + 1).padStart(2, "0")}-15` }] }),
    useSettings: () => ({ data: { exchange_rate_jpy_to_cny: 0.06 } }),
  };
});

beforeEach(() => { vi.clearAllMocks(); api.pending = false; api.create.mockResolvedValue({}); api.update.mockResolvedValue({}); });
afterEach(cleanup);
