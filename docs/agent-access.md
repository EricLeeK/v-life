...
deno test --no-lock supabase/functions/mcp-server/mcpAdapter.test.ts
...
未登录 `/mcp` 应返回 401，`WWW-Authenticate` 指向可读取的 OAuth metadata，而不是网关的 Missing authorization header。登录后验证初始化、工具发现、读取、创建、修改、删除、重复创建、分页导出及撤销后的 403。
